...
from sensing_targets import (
    OsCfarParams,
    build_direct_targets as shared_build_direct_targets,
    cluster_peak_points as shared_cluster_peak_points,
    cluster_detected_targets as shared_cluster_detected_targets,
    run_os_cfar_2d as shared_run_os_cfar_2d,
)
from sensing_viewer.runtime import FrameBuffer
from sensing_viewer.config import default_config

# ====== Backend Detection ======
from sensing_viewer.array_backend import (
    BACKEND_NAME,
    FORCED_BACKEND,
    HAVE_NUMBA,
    USE_APPLE_GPU,
...
VIEWER_MODE = os.environ.get("OPENISAC_FAST_VIEWER_MODE", "mono").strip().lower()
if VIEWER_MODE not in {"mono", "bi"}:
    VIEWER_MODE = "mono"
VIEWER_CONFIG = default_config(VIEWER_MODE)
IS_BI_MODE = VIEWER_CONFIG.mode == "bi"
SETTINGS_KEY = VIEWER_CONFIG.settings_key
DEFAULT_DATA_PORT = VIEWER_CONFIG.default_port
DEFAULT_CONTROL_PORT = VIEWER_CONFIG.default_control_port
DEFAULT_CHANNEL_COUNT = VIEWER_CONFIG.default_channels
...
local_detector_mode = (
    LOCAL_DETECTOR_OS_CFAR
    if VIEWER_CONFIG.supports_os_cfar
    else LOCAL_DETECTOR_CLEAN
)
...
TARGET_SECTOR_DEFAULT_MIN_RANGE_BIN = 0
TARGET_SECTOR_DEFAULT_MAX_RANGE_BINS = 100
DISPLAY_TIMER_INTERVAL_MS = 5

RAW_QUEUE_SIZE = 20
DISPLAY_QUEUE_SIZE = 5
DISPLAY_DOWNSAMPLE = VIEWER_CONFIG.display_downsample
BUFFER_LENGTH = 5000
...
def get_os_cfar_params():
    return OsCfarParams(
        train_doppler=max(0, int(cfar_train_doppler)),
        train_range=max(0, int(cfar_train_range)),
        guard_doppler=max(0, int(cfar_guard_doppler)),
        guard_range=max(0, int(cfar_guard_range)),
        alpha_db=float(cfar_alpha_db),
        min_range_bin=max(0, int(cfar_min_range_bin)),
        dc_exclusion_bins=max(0, int(cfar_dc_exclusion_bins)),
        min_power_db=float(cfar_min_power_db),
        max_points=max(1, int(cfar_max_points)),
        rank_percent=float(cfar_os_rank_percent),
        suppress_doppler=max(0, int(cfar_os_suppress_doppler)),
        suppress_range=max(0, int(cfar_os_suppress_range)),
    )
...
def run_os_cfar_2d(
    ...
    return shared_run_os_cfar_2d(
        to_cpu_array(rd_db, dtype=np.float32),
        get_os_cfar_params(),
        active_row_start=active_row_start,
        active_row_stop=active_row_stop,
        active_col_start=active_col_start,
        active_col_stop=active_col_stop,
        dc_center_row=dc_center_row,
    )


def cluster_detected_targets(cfar_points, rd_data, point_strengths_db=None):
    return shared_cluster_detected_targets(
        cfar_points,
        rd_data,
        point_strengths_db=point_strengths_db,
        eps_doppler=get_dbscan_eps_doppler(),
        eps_range=get_dbscan_eps_range(),
        min_samples=max(1, int(target_dbscan_min_samples)),
    )


def build_direct_targets(points, rd_data, point_strengths_db=None):
    return shared_build_direct_targets(
        points,
        rd_data,
        point_strengths_db=point_strengths_db,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description=f"{VIEWER_CONFIG.mode} sensing viewer (fast)")
    parser.add_argument(
        "--host",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        help="Logical channel count carried by the sensing stream",
    )
    parser.add_argument(
        "--control-port",
        type=int,
...
if not CHANNELS:
    CHANNELS = [ChannelRuntime(0, DEFAULT_DATA_PORT, args.control_port)]
...
control_socket = make_control_dealer(CONTROL_ENDPOINT, identity=f"{CONTROL_IDENTITY_PREFIX}-{os.getpid()}")
...
def reconnect_backend(host):
    ...
    new_socket = make_control_dealer(
        new_endpoint,
        identity=f"{CONTROL_IDENTITY_PREFIX}-{os.getpid()}-{int(time.time() * 1000)}",
    )
    ...
    while running:
        if local_generation != connection_generation:
            ...
            try:
                ...
                print(f"{STREAM_PREFIX} Sensing SUB connect error: {e}")
                _time.sleep(0.5)
                continue
            poller = zmq.Poller()
            ...
            local_generation = connection_generation
            logged_waiting_for_params = False
            print(f"{STREAM_PREFIX} Subscribed to {STREAM_DESCRIPTION} stream at {endpoint}")
        ...
        if sock not in events:
            ...
            _request_params_if_needed(f"{STREAM_PREFIX} Viewer params not yet received; requesting from backend...")
            continue
        ...
        try:
            ...
            if IS_BI_MODE:
                ...
                if not data_payload:
                    continue
                frame_counter += 1
                decoded = decode_sensing_payload(frame_counter, data_payload, viewer_params)
                _queue_aggregate_frame(decoded.frame_id, [decoded.matrix])
                continue
            ...
            if metadata_payload is not None or backend_stream_unsupported(viewer_params):
                warn_unsupported_backend_payload(STREAM_PREFIX, "aggregate sensing frame")
                continue
            if not data_payload:
                continue
...
def _finalize_channel_result(
    ...
    cluster_points = shared_cluster_peak_points(target_clusters)
    md_spectrum = None
    md_extent = None
...
def send_alignment_command(val):
    ...
    if target is None:
        return
    if IS_BI_MODE:
        send_shared_control_command(b"ALGN", int(val))
        return
...
def send_system_response_calibration_command():
    ...
    if target is None:
        return
    if IS_BI_MODE:
        send_shared_control_command(CALIBRATE_SYSTEM_RESPONSE_COMMAND, 0)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(VIEWER_CONFIG.title)
        ...
        self.target_sector_min_range_bin = TARGET_SECTOR_DEFAULT_MIN_RANGE_BIN
        ...
        self.target_sector_controls = QtWidgets.QWidget()
        sector_range_layout = QtWidgets.QHBoxLayout(self.target_sector_controls)
        sector_range_layout.setContentsMargins(0, 0, 0, 0)
        sector_range_layout.addWidget(QtWidgets.QLabel("Sector Min (bin):"))
        self.txt_sector_min_bin = QtWidgets.QLineEdit(str(self.target_sector_min_range_bin))
        self.txt_sector_min_bin.setToolTip("Minimum range bin used as the origin of the target sector view.")
        sector_range_layout.addWidget(self.txt_sector_min_bin)
        ...
        targets_layout.addWidget(self.target_sector_controls)
        ...
        self.rd_cfar_marker = pg.ScatterPlotItem(
            [], [], symbol='o', size=4,
            pen=pg.mkPen(185, 28, 28, 230, width=1.0),
            brush=pg.mkBrush(239, 68, 68, 220)
        )
        ...
        self._settings = ViewerSettings(SETTINGS_KEY)
        ...
        self.display_channel_selector = QtWidgets.QWidget()
        ch_select_layout = QtWidgets.QHBoxLayout(self.display_channel_selector)
        ch_select_layout.setContentsMargins(0, 0, 0, 0)
        ...
        disp.addWidget(self.display_channel_selector)
        ...
        self.tx_gain_controls = QtWidgets.QWidget()
        tx_gain_layout = QtWidgets.QHBoxLayout(self.tx_gain_controls)
        tx_gain_layout.setContentsMargins(0, 0, 0, 0)
        ...
        hw_sec.addWidget(self.tx_gain_controls)

        self.rx_gain_controls = QtWidgets.QWidget()
        rx_gain_layout = QtWidgets.QHBoxLayout(self.rx_gain_controls)
        rx_gain_layout.setContentsMargins(0, 0, 0, 0)
        ...
        hw_sec.addWidget(self.rx_gain_controls)
        ...
        self.aoa_freq_controls = QtWidgets.QWidget()
        aoa_freq_layout = QtWidgets.QHBoxLayout(self.aoa_freq_controls)
        aoa_freq_layout.setContentsMargins(0, 0, 0, 0)
        ...
        hw_sec.addWidget(self.aoa_freq_controls)

        self.aoa_spacing_controls = QtWidgets.QWidget()
        aoa_spacing_layout = QtWidgets.QHBoxLayout(self.aoa_spacing_controls)
        aoa_spacing_layout.setContentsMargins(0, 0, 0, 0)
        ...
        hw_sec.addWidget(self.aoa_spacing_controls)
    ...
    def _angle_ui_available(self):
        return bool(VIEWER_CONFIG.supports_phase_calibration and len(CHANNELS) >= 2)

    def _apply_mode_visibility(self):
        angle_ui = self._angle_ui_available()
        multi_channel = len(CHANNELS) > 1

        for attr, visible in (
            ("display_channel_selector", multi_channel),
            ("phase_curve_plot", angle_ui),
            ("lbl_phase_sync", angle_ui),
            ("lbl_phase_clicked", angle_ui),
            ("lbl_aoa_status", angle_ui),
            ("target_sector_controls", angle_ui),
            ("target_sector_plot", angle_ui),
            ("section_superres", VIEWER_CONFIG.supports_superresolution),
            ("tx_gain_controls", VIEWER_CONFIG.supports_gain_control),
            ("rx_gain_controls", VIEWER_CONFIG.supports_gain_control),
            ("aoa_freq_controls", angle_ui),
            ("aoa_spacing_controls", angle_ui),
            ("btn_calibrate", angle_ui),
        ):
            if hasattr(self, attr):
                getattr(self, attr).setVisible(bool(visible))

        if not VIEWER_CONFIG.supports_os_cfar:
            for attr, visible in (
                ("local_detector_selector", False),
                ("local_os_cfar_controls", False),
                ("local_clean_controls", True),
            ):
                if hasattr(self, attr):
                    getattr(self, attr).setVisible(bool(visible))
    ...
    def _load_and_apply_settings(self):
        ...
        sector_min_saved = self._settings.get("sector_min_bin", self._settings.get("sector_zero_bin", None))
        if sector_min_saved is not None:
            self.txt_sector_min_bin.setText(str(sector_min_saved))
        ...
        if not VIEWER_CONFIG.supports_superresolution:
            delay_estimator_mode = DELAY_ESTIMATOR_FFT
            self.combo_delay_estimator.blockSignals(True)
            self.combo_delay_estimator.setCurrentIndex(self.combo_delay_estimator.findData(DELAY_ESTIMATOR_FFT))
            self.combo_delay_estimator.blockSignals(False)
        ...
        if not VIEWER_CONFIG.supports_os_cfar:
            local_detector_mode = LOCAL_DETECTOR_CLEAN
            self.combo_local_detector.blockSignals(True)
            self.combo_local_detector.setCurrentIndex(self.combo_local_detector.findData(LOCAL_DETECTOR_CLEAN))
            self.combo_local_detector.blockSignals(False)
    ...
    def _connect_settings_persistence(self):
        ...
        _sv("sector_min_bin", self.txt_sector_min_bin)
    ...
    def refresh_detector_controls(self, force=False):
        ...
        self.superres_controls.setVisible(VIEWER_CONFIG.supports_superresolution)
        self.local_detector_controls.setVisible(not using_superres)
        self.local_detector_selector.setVisible((not using_superres) and VIEWER_CONFIG.supports_os_cfar)
        ...
        self.local_os_cfar_controls.setVisible(
            (not using_superres)
            and VIEWER_CONFIG.supports_os_cfar
            and local_detector_mode == LOCAL_DETECTOR_OS_CFAR
        )
    ...
    def refresh_display_button_text(self):
        ...
        self.phase_curve_plot.setVisible(self._angle_ui_available())
        self._refresh_phase_calibration_button()
        ...
        if not cfar_enabled:
            for ch in CHANNELS:
                if isinstance(ch.current_display_data, dict):
                    ...
                    ch.current_display_data['cluster_points'] = np.empty((0, 2), dtype=np.int32)
        ...
        try:
            ...
            for ch in CHANNELS:
                if isinstance(ch.current_display_data, dict):
                    ...
                    ch.current_display_data.pop('cluster_points', None)
            self._last_top_targets_key = None
            self._force_ui_refresh = True
            reset_processing_state()
            print(
    ...
    def set_target_sector_range(self):
        try:
            min_bin = max(0, int(self.txt_sector_min_bin.text()))
            max_bin = max(1, int(self.txt_sector_max_bin.text()))
            self.target_sector_min_range_bin = min_bin
            self.target_sector_max_range_bins = max_bin
            self.txt_sector_min_bin.setText(str(self.target_sector_min_range_bin))
            self.txt_sector_max_bin.setText(str(self.target_sector_max_range_bins))
            self._settings.set("sector_min_bin", str(self.target_sector_min_range_bin))
            self._settings.set("sector_max_bin", str(self.target_sector_max_range_bins))
            self._settings.save_now()
            print(
                "Target sector range updated: "
                f"min_range_bin={self.target_sector_min_range_bin}, "
            ...
            print(
                ...
                f"min={self.txt_sector_min_bin.text()} max={self.txt_sector_max_bin.text()}"
            )
            self.txt_sector_min_bin.setText(str(self.target_sector_min_range_bin))
            self.txt_sector_max_bin.setText(str(self.target_sector_max_range_bins))
    ...
    def _save_phase_calibration_data(
        ...
        try:
            ...
            payload = {
                ...
                'target_sector_min_range_bin': np.array(int(self.target_sector_min_range_bin), dtype=np.int32),
    ...
    def _format_target_sector_scale_text(self, base_scale_text):
        return (
            f"{base_scale_text}; min=R{self.target_sector_min_range_bin}; "
    ...
    def _get_overlay_cluster_points(self, latest_disp):
        if not latest_disp:
            return np.empty((0, 2), dtype=np.int32)
        cluster_points = latest_disp.get('cluster_points')
        if cluster_points is not None:
            return np.asarray(cluster_points, dtype=np.int32)
        clusters = self._get_target_clusters(latest_disp)
        cluster_points = shared_cluster_peak_points(clusters)
        latest_disp['cluster_points'] = cluster_points
    ...
    def _update_target_sector_plot(self, entries, axis_unit, scale_text, range_scale):
        ...
        if not valid_entries:
            ...
            self.target_sector_plot.setTitle(
                f"Target Sector View [{scale_text}; AoA pending or target before min bin]"
            )
            return
    ...
    def _build_top_target_entries(self, ch, latest_disp):
        ...
        for rank, cluster in enumerate(clusters[:TARGET_TEXT_POINT_LIMIT], start=1):
            ...
            relative_range_bins = float(r_idx) - float(self.target_sector_min_range_bin)
    ...
    def update_plots(self):
        ...
        if ch is None:
            ...
            top_targets_key = (
                ...
                int(self.target_sector_min_range_bin),
            ...
            if str(detector_mode).startswith("delay_superres_"):
                ...
                if cfar_enabled:
                    cfar_status += (
                        ...
                        f" dbscan=({get_dbscan_eps_doppler()},{get_dbscan_eps_range()},{target_dbscan_min_samples})"
                ...
                if cfar_enabled:
                    cfar_status_short += (
                        ...
                        f" db={get_dbscan_eps_doppler()}/{get_dbscan_eps_range()}/{target_dbscan_min_samples}"
                    )
            else:
